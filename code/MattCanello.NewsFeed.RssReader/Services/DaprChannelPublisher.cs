...
namespace MattCanello.NewsFeed.RssReader.Services
{
    public sealed class DaprChannelPublisher : IChannelPublisher
    {
        ...
        public async Task PublishChannelUpdatedAsync(string channelId, Channel channel, CancellationToken cancellationToken = default)
        {
            ArgumentNullException.ThrowIfNull(channelId);
            ArgumentNullException.ThrowIfNull(channel);

            var cloudEvent = _cloudEventFactory.CreateChannelUpdatedEvent(channelId, channel);

            var cloudEventData = _cloudEventFormatter.EncodeStructuredModeMessage(cloudEvent, out _);

            var bindingRequest = CreateBindingRequest(channelId, cloudEventData);

            await _daprClient.InvokeBindingAsync(bindingRequest, cancellationToken);
        }

        public static BindingRequest CreateBindingRequest(string channelId, ReadOnlyMemory<byte> data)
        {
            ...
            bindingRequest.Metadata["partitionKey"] = channelId;
            bindingRequest.Metadata["PartitionKey"] = channelId;
